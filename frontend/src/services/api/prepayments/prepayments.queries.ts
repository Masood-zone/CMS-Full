...
export const useFetchPrepaymentsByClass = (classId: number) => {
  return useQuery(
    ["prepayments", classId],
    () => prepaymentsApi.fetchPrepaymentsByClass(classId),
    {
      onError: (error) => {
        console.error(error);
...
export const useCreatePrepayment = () => {
  ...
  return useMutation(
    (data: CreatePrepayment) => prepaymentsApi.createPrepayment(data),
    {
      onSuccess: () => {
...
export const useUpdatePrepayment = () => {
  const queryClient = useQueryClient();
  return useMutation(
    (data: UpdatePrepayment) => prepaymentsApi.updatePrepayment(data.id, data),
    {
      onSuccess: () => {
