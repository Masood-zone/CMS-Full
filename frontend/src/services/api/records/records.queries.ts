...
import type { StudentRecord } from "./records.types" // Declare the StudentRecord variable

/**
 * Query: Fetch all records.
 */
export const useFetchRecords = () => {
  return useQuery(["records"], recordsApi.fetchRecords, {
    onError: (error) => {
      console.error(error)
 ...
 */
export const useFetchRecordsByClassAndDate = (classId: number, date: string) => {
  return useQuery(["records", classId, date], () => recordsApi.fetchRecordsByClassAndDate(classId, date), {
    onError: (error) => {
      console.error(error)
      toast.error("Failed to fetch records.")
    },
 ...
 */
export const useStudentRecordsByClassAndDate = (classId: number, date: string) => {
  return useQuery(["studentRecords", classId, date], () => recordsApi.getStudentRecordsByClassAndDate(classId, date), {
...
export const useFetchRecordsDetail = (id: number) => {
  ...
  return useQuery(["recordDetails", id], () => recordsApi.fetchRecordDetails(id), {
    onSuccess: () => {
...
export const useUpdateStudentStatus = () => {
  const queryClient = useQueryClient()
  return useMutation((data: StudentRecord) => recordsApi.updateStudentStatus(data), {
    onSuccess: () => {
