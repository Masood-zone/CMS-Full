...
export class RecordService {
  ...
  async generateDailyRecords(params: GenerateDailyRecordsParams) {
    ...
    // Get classes
    const classQuery = classId ? { where: { id: Number(classId), isActive: true } } : { where: { isActive: true } }
    ...
    for (const classItem of classes) {
      for (const student of classItem.students) {
        try {
          const prepayment = activePrepayments.find((p) => p.studentId === student.id)
  ...
  async submitRecord(recordData: any) {
    ...
    const adminExists = await prisma.user.findUnique({ where: { id: Number(submittedBy) } })
    if (!adminExists) {
    ...
    const updatedRecords = await prisma.$transaction(
      allStudents.map((student) => {
        const prepayment = activePrepayments.find((p) => p.studentId === Number(student.paidBy))
        ...
        return prisma.record.upsert({
          ...
          update: {
            ...
            isAbsent: absentStudents.some((s: any) => s.paidBy === student.paidBy),
  ...
  async updateRecord(id: number, updateData: any) {
    const { amount, payedBy, isPrepaid, hasPaid, adminId, classId, isAbsent, paymentType, notes } = updateData
  ...
  async updateStudentStatus(id: number, statusData: { hasPaid: boolean; isAbsent: boolean }) {
