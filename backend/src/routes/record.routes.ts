...
router.get(
  "/submitted",
  asyncHandler(recordController.getSubmittedRecordsByDate)
);
router.get("/details", asyncHandler(recordController.getRecordDetails));
router.get(
  "/:classId",
  asyncHandler(recordController.getStudentRecordsByClassAndDate)
);
...
router.post(
  "/submit",
  validate(recordValidation.submit),
  asyncHandler(recordController.submitRecord)
);

router.put(
  "/:id",
  validate(recordValidation.update),
  asyncHandler(recordController.update)
);
router.put(
  "/:id/status",
  validate(recordValidation.updateStatus),
  asyncHandler(recordController.updateStatus)
);

router.delete(
  "/:id",
  authorize("ADMIN", "SUPER_ADMIN"),
  asyncHandler(recordController.delete)
);
